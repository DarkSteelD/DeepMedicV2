...
class AccuracyMonitorForEpSegm(object):
    ...
    def update_metrics_after_subep(self, meanCostOfSubepoch, perClassRpRnTpTnInSubep):
        ...
        # Add protection for division by zero in overall accuracy
        ...
        for class_i in range(self.numberOfClasses) :
            ...
            meanAccuracyClassVsAllOfSubep = self.NA_PATTERN if (numOfRealPosInSubep + numOfRealNegInSubep) == 0 else \
                                           (numOfTruePosInSubep + numOfTrueNegInSubep) / float(numOfRealPosInSubep + numOfRealNegInSubep)
            meanAccuracyOnPosOfSubep = self.NA_PATTERN if numOfRealPosInSubep == 0 else numOfTruePosInSubep*1.0/numOfRealPosInSubep
            meanPrecOfSubep = self.NA_PATTERN if (numOfTruePosInSubep + numOfFalsePosInSubep) == 0 else \
                             numOfTruePosInSubep*1.0/(numOfTruePosInSubep + numOfFalsePosInSubep)
            ...
            meanDiceOfSubep = self.NA_PATTERN if (numOfPredPosInSubep + numOfRealPosInSubep) == 0 else \
                             (2.0*numOfTruePosInSubep)/(numOfPredPosInSubep + numOfRealPosInSubep)
        ...
        # Add protection for division by zero in foreground metrics
        foregrMeanAccOfSubep = self.NA_PATTERN if (foregrRp + foregrRn) == 0 else \
                              (foregrTp + foregrTn) / float(foregrRp + foregrRn)
        foregrMeanAccOnPosOfSubep = self.NA_PATTERN if foregrRp == 0 else foregrTp*1.0/foregrRp
        foregrMeanPrecOfSubep = self.NA_PATTERN if (foregrTp + foregrFp) == 0 else \
                               foregrTp*1.0 / (foregrTp + foregrFp)
        ...
        foregrMeanDiceOfSubep = self.NA_PATTERN if (foregrPredPosInSubep + foregrRp) == 0 else \
                               (2.0*foregrTp)/(foregrPredPosInSubep + foregrRp)
